...
def create_survey(db: Session, survey: SurveyCreate) -> Survey:
    db_survey = Survey(
        ...
        translations=getattr(survey, "translations", {}),
        ...
        ai_generated=getattr(survey, "ai_generated", False),
        adaptive_config=getattr(survey, "adaptive_config", {}),
        audio_metadata=getattr(survey, "audio_metadata", None),
    )
    db.add(db_survey)
    db.commit()
